...
public class ZoomImageView extends View {
	...
	/**
	 * 修改高度中心比例. 默认是一半 heightRation = 2f
	 * @param heightRatio
	 */
	...
	private void zoom(Canvas canvas) {
		...
		if (currentBitmapHeight < height) {
		    //heightRotio 2.0f
			translateY = (height - scaledHeight) / heightRatio;
		} else {
		...
		if (sourceBitmap != null) {
			...
			if (bitmapWidth > width || bitmapHeight > height) {
				if (bitmapWidth - width > bitmapHeight - height) {
					...
					//2.0f 改成2.5f
					float translateY = (height - (bitmapHeight * ratio)) / heightRatio;
                ...
                //自定义逻辑当，图片宽度小于高度证明是一个横图。图片放大到屏幕宽度
                if(bitmapWidth >= bitmapHeight){
                    ...
                    // 当图片的宽高都小于屏幕宽高时，并且高度大于宽度直接让图片居中显示
                    float translateX = (width - sourceBitmap.getWidth()) / 2f;
                    //2.0f 改成2.5f
                    float translateY = (height - sourceBitmap.getHeight()) / heightRatio;
                    matrix.postTranslate(translateX, translateY);
                    totalTranslateX = translateX;
                    totalTranslateY = translateY;
