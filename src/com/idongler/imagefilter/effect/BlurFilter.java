...
public class BlurFilter implements IImageFilter {
    ...
    public Image process(Image imageIn) {
        ...
        int w = imageIn.image.getWidth();
        int h = imageIn.image.getHeight();
        int wm = imageIn.image.getWidth() - 1;
        int hm = imageIn.image.getHeight() - 1;
        int wh = imageIn.image.getWidth() * imageIn.image.getHeight();
        ...
        for (y = 0; y < h; y++) {
            ...
            for (i = -mRadius; i <= mRadius; i++) {
                p = imageIn.colorArray[yi + Math.min(wm, Math.max(i, 0))];
                sir = stack[i + mRadius];
                sir[0] = (p & 0xff0000) >> 16;
                sir[1] = (p & 0x00ff00) >> 8;
                sir[2] = (p & 0x0000ff);
            ...
            for (x = 0; x < w; x++) {
                ...
                p = imageIn.colorArray[yw + vmin[x]];
        ...
        for (x = 0; x < w; x++) {
            ...
            for (y = 0; y < h; y++) {
                ...
                imageIn.colorArray[yi] = (0xff000000 & imageIn.colorArray[yi]) | (dv[rsum] << 16) | (dv[gsum] << 8) | dv[bsum];
