...
public class PostThirdLoginNewService implements HttpAysnTaskInterface {
	...
	public PostThirdLoginNewService(Context context, Integer mTag,
	...
	public void doPostThirdLogin(final Map<String, String> info) {
		try {

			if (!SystemTool.checkNet(context)) {
				AppToast.toastMsgCenter(context,
			...
			final String skin_score_url = context.getResources().getString(
					R.string.v1_third)+"?client=2";
			// final JSONObject param = new JSONObject();
			// for(String key : info.keySet()){
			// param.put(key, info.get(key)+"");
			// }
			// StringEntity bodyEntity = new
			// StringEntity(param.toString(),"UTF-8");
			// Map<String,String> map = new HashMap<String,String>();
			// map.put("Content-Type", "application/json");
			// //请求
			// HttpClientUtils client=new HttpClientUtils();
			// client.post_with_head_and_body(context, mTag, skin_score_url,
			// map, bodyEntity, PostThirdLoginNewService.this);
				...
				public void run() {
					...
					try {
						...
						HttpPost httppost = new HttpPost(
								HttpClientUtils.BASE_URL + skin_score_url);
						httppost.addHeader("Content-Type", "application/json");
						JSONObject paramJson = new JSONObject();
						for (String key : info.keySet()) {
							paramJson.put(key, info.get(key) + "");
						}
						...
						if (code == 200 || code == 201) {
							String rev = EntityUtils.toString(response
									.getEntity());
							PostThirdLoginNewService.this.requestComplete(
									PostThirdLoginNewService.this.mTag, code,
									response.getAllHeaders(), rev,
									true);
							...
							PostThirdLoginNewService.this.requestComplete(
									PostThirdLoginNewService.this.mTag, code,
									response.getAllHeaders(), "error", false);
						}
					} catch (Exception e) {
						PostThirdLoginNewService.this.requestComplete(
								PostThirdLoginNewService.this.mTag, 400, null,
								"error", false);
	...
	public void requestComplete(Object tag, int statusCode, Object header,
		...
		if (null != this.callback) {
			...
			this.callback.dataCallBack(tag, statusCode,
					parseJsonObject(header, body));
		}
	}
	...
	public UserEntity parseJsonObject(Object header, Object result) {
		try {
			...
			ACache.get(context).put("integral_real", obj.optInt("integral_real"));//放到cache中
			ACache.get(context).put("type", "" + obj.optString("type"));
			ACache.get(context).put("avatar",
					obj.optString("avatar").replaceAll("\\\\", "") + "");
