...
public class Msg {
	...
	public String getStatusDesc() {
		if (status == Status.SendStart) {
			return MyApplication.ctx.getString(R.string.sending);
		} else if (status == Status.SendReceived) {
			return MyApplication.ctx.getString(R.string.received);
		} else if (status == Status.SendSucceed) {
			return MyApplication.ctx.getString(R.string.sent);
		} else if (status == Status.SendFailed) {
			return MyApplication.ctx.getString(R.string.failed);
		} else if (status == Status.HaveRead) {
			return MyApplication.ctx.getString(R.string.haveRead);
		} else {
	...
	public CharSequence getNotifyContent() {
		switch (type) {
		case Audio:
			return MyApplication.ctx.getString(R.string.audio);
		case Text:
			if (EmotionUtils.haveEmotion(getContent())) {
				return MyApplication.ctx.getString(R.string.emotion);
			} else {
				return getContent();
			}
		case Image:
			return MyApplication.ctx.getString(R.string.image);
		case Location:
			return MyApplication.ctx.getString(R.string.position);
		default:
			return MyApplication.ctx.getString(R.string.newMessage);
		}
	}
